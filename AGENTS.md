...
- **Project `nub.jsonc` is an intentional config surface and an explicit naming exception** — nub's canonical typed runtime/install/dlx configuration, separate from global `~/.config/nub/nub.jsonc`. Its parser and disabled discovery gate land before the user-facing release: do not remove the reader as dead code, and do not unflag discovery incidentally. Sandbox values may be parsed while remaining inert until their dedicated frontends ship.
