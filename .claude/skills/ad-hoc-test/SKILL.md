...
- **Read the artifact you BUILT, not the first path that matches its name.** Binding the change to the binary (above) is only half of it — you must also confirm the file you inspect is that binary's. `--profile fast` writes `target/fast/`, `cargo test` writes `target/debug/`, and generated files live under a per-crate `build/<crate>-<hash>/out/`, so several plausible copies coexist and the stale one is often first alphabetically. Sort candidates by mtime and check the winner is newer than the build. Burned 2026-07-31: a generated const grepped out of `target/debug/` reported 111 new entries **ABSENT** after a green `--profile fast` build; the fresh copy under `target/fast/` had all of them, and the near-miss was a bug report filed against a correct change.
...
- **Exit code is NOT a universal success condition — check what the script actually does first.** Some lifecycle scripts wrap `spawnSync` in `try/catch` and never propagate its status, so *every* arm exits 0 no matter what was denied. Every hook installer measured (`lefthook`, `@arkweid/lefthook`, `@evilmartians/lefthook`) behaves this way, and a grant ladder scored on rc alone reads uniformly green while the hook is never written. Where that happens, judge the ARTIFACT against a jail-off control instead. Exit code remains the right default; this is a real exception class, not a reason to abandon it.
- **A grant can MOVE a failure rather than close it.** Re-measure after every grant instead of assuming one is the whole job. `lefthook` needed three staged grants — git detection, then its own config read, then a checksum write — each invisible until the previous one landed, and the brief that said "the first is very likely the whole job" was wrong.
