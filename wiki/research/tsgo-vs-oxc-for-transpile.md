...
- **Failure modes:** Daemon crash mid-build, version skew between daemon and CLI, PATH-shim child workers ([`pm-run-compat-scope.md`](pm-run-compat-scope.md)) needing to discover the same daemon, security model (which user owns the socket, where does it live, what happens under `--permission`).
